defmodule Tests.EdgeDB.Protocol.Codecs.Flaot64Test do
  ...
  test "decoding std::float64 number", %{conn: conn} do
    value = 0.5
    assert {:ok, ^value} = EdgeDB.query_single(conn, "SELECT <float64>0.5")
  end

  test "decoding NaN as std::float64 number", %{conn: conn} do
    value = :nan
    assert {:ok, ^value} = EdgeDB.query_single(conn, "SELECT <float64>'NaN'")
  end

  test "decoding infinity as std::float64 number", %{conn: conn} do
    value = :infinity
    assert {:ok, ^value} = EdgeDB.query_single(conn, "SELECT <float64>'inf'")
  end
  ...
  test "encoding std::float64 argument", %{conn: conn} do
    value = 1.0
    assert {:ok, ^value} = EdgeDB.query_single(conn, "SELECT <float64>$0", [value])
  end
