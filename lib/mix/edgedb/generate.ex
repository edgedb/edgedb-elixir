...
defmodule Mix.Tasks.Edgedb.Generate do
  @validator_definition [
    ...
    credentials_file: [
      type: :string,
      doc: "the path to the instance credentials file containing the instance parameters to connect to."
    ],
    instance: [
      type: :string,
  ...
  def run(args) do
    ...
    with {args, [], []} <- OptionParser.parse(args, strict: parser_definition()),
      ...
      {_args, _unparsed, errors} ->
        Mix.shell().error("Unable to parse arguments: #{inspect(Enum.map(errors, fn {key, _value} -> key end))}")
      ...
      {:error, %NimbleOptions.ValidationError{key: key} = error} ->
        Mix.shell().error("Error while validating #{inspect(key)} argument: #{Exception.message(error)}")
