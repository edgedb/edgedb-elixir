defmodule EdgeDB.EdgeQL.Generator do
  ...
  defp generate_elixir_module(%EdgeDB.Query{} = query, query_file, output_file, module_name) do
    ...
    args = input_codec_to_args(input_codec, query.codec_storage, module_name)
    shape = output_codec_to_shape(query, output_codec, query.codec_storage, module_name)
  ...
  defp input_codec_to_args(%Codecs.Null{}, _codec_storage, _module_name) do
    %Generator.Args{is_empty: true}
  end

  defp input_codec_to_args(%Codecs.Object{} = codec, codec_storage, module_name) do
    positional? =
      ...
      |> Enum.reduce([], fn {element, codec}, args ->
        ...
        [
          %Generator.Args.Arg{
            ...
            type: handle_codec(codec, codec_storage, module_name)
          }
          | args
        ]
      end)
  ...
  defp output_codec_to_shape(%EdgeDB.Query{} = query, codec, codec_storage, module_name) do
    %Generator.Shape{
  ...
  defp handle_codec(%Codecs.Object{} = codec, codec_storage, module_name) do
    fields =
      codec.shape_elements
      |> Enum.zip(codec.codecs)
  ...
  defp handle_codec(%Codecs.Set{} = codec, codec_storage, module_name) do
  ...
  defp handle_codec(%Codecs.UUID{}, _codec_storage, module_name) do
    typename = "uuid()"
  ...
  defp handle_codec(%Codecs.JSON{}, _codec_storage, module_name) do
    typename = "json()"
  ...
  defp handle_codec(%Codecs.Duration{}, _codec_storage, module_name) do
  ...
  defp handle_codec(%Codecs.Vector{}, _codec_storage, module_name) do
    typename = "vector()"
  ...
  defp handle_codec(%codec_name{}, _codec_storage, _module_name) when codec_name in @scalar_codecs do
    %Generator.Scalar{typespec: elem(@builtin_scalars_to_typespecs[codec_name], 1)}
  end

  defp handle_codec(%Codecs.Scalar{codec: subcodec, name: nil}, codec_storage, _module_name) do
  ...
  defp handle_codec(%Codecs.Scalar{codec: subcodec, name: type_name}, codec_storage, module_name) do
    %subcodec_name{} = CodecStorage.get(codec_storage, subcodec)
  ...
  defp handle_codec(%Codecs.Enum{name: type_name, members: members}, _codec_storage, module_name) do
    full_type_name = full_name_to_typespec(type_name)
    typedoc = "scalar type #{type_name} extending enum<#{Enum.join(members, ", ")}>"

    register_typespec(
      full_type_name,
      {typedoc, Enum.map(members, &":#{inspect(&1)}")}
    )

    %Generator.Enum{typespec: full_type_name, module: module_name, members: members}
  end

  defp handle_codec(%Codecs.Array{codec: subcodec}, codec_storage, module_name) do
    subcodec = CodecStorage.get(codec_storage, subcodec)
    type = handle_codec(subcodec, codec_storage, module_name)
    %Generator.Array{type: type}
  end

  defp handle_codec(%Codecs.Tuple{codecs: subcodecs}, codec_storage, module_name) do
    elements =
      Enum.map(subcodecs, fn subcodec ->
        subcodec = CodecStorage.get(codec_storage, subcodec)
        handle_codec(subcodec, codec_storage, module_name)
      end)

    %Generator.Tuple{elements: elements}
  end

  defp handle_codec(%Codecs.NamedTuple{codecs: subcodecs, elements: elements}, codec_storage, module_name) do
    elements =
      subcodecs
      |> Enum.zip(elements)
  ...
  defp handle_codec(%Codecs.Range{codec: subcodec}, codec_storage, module_name) do
    subcodec = CodecStorage.get(codec_storage, subcodec)
    type = handle_codec(subcodec, codec_storage, module_name)
    %Generator.Range{type: type}
  end

  defp handle_codec(%Codecs.MultiRange{codec: subcodec}, codec_storage, module_name) do
    subcodec = CodecStorage.get(codec_storage, subcodec)
    type = handle_codec(subcodec, codec_storage, module_name)
